...
`---`
`Generated by [proof-engine](https://github.com/yaniv-golan/proof-engine) on [date].`
...
- The `generator` block must appear in the JSON summary with `name`, `repo`, and `generated_at` fields
