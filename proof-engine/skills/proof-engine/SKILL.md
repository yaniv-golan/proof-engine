...
- **WebFetch paraphrases quotes**: WebFetch and similar AI-powered fetch tools return processed summaries, not raw page text. Quotes obtained from WebFetch may be silently reformatted (parentheses → dashes, "circa" → "~", sentence splicing). Always verify quotes are verbatim before writing them into `empirical_facts` — either re-fetch the raw page or check against a `snapshot`. Expect a correction loop if the first quote fails `verify_all_citations`.
